...
// genuinely do.
export const MOUNT_SHARED_APPS = new Set([
...
/**
 * Picks scheme based on the main service's published ports.
 * If port 443 is exposed → "https", otherwise "http".
 */
export function pickScheme(services, mainKey) {
...
/**
 * Returns true if the image string has an explicit non-:latest tag.
 * Bare names ("vendor/image") and ":latest" are rejected.
 */
export function isImagePinned(img) {
  ...
  if (process.argv.includes("--verbose")) {
    ...
    if (summary.manualReview.missingPortMap.length) {
      ...
      for (const e of summary.manualReview.missingPortMap) {
        console.log(
          `  ${e.app} (main=${e.main}): ports=[${e.ports.join(", ")}]`,
        );
      }
    }
